...
class BrandController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->baseData['moduleKey'] = 'brand';
        $this->baseData['baseRouteName'] = $this->baseData['baseRouteName'] . '.' . $this->baseData['moduleKey'] . '.';
        $this->baseData['trans_text'] = BrandHelper::getLang();
    }
    ...
    {
        $this->baseData['allData'] = Brand::orderBy('created_at', 'desc')->paginate(25);
...
//        return  $this->baseData['allData'];
    ...
    {
        try {
            $this->baseData['routes']['create_form_data'] = BrandHelper::getRoutes()['create_data'];
    ...
    {
        try {
            $this->baseData['routes'] = BrandHelper::getRoutes();
...
//            $this->baseData['options']['galleries_attributes'] = config('blog.galleries_attributes');
            ...
            if ($request->get('id')) {
                $item = Brand::findOrFail($request->get('id'))->load(['translations']);
                $locales = TranslationFieldsWithLocale::setTranslations($item->translations)->setTranslationFields((new BrandTranslation())->getFillable())
...
//                ->setGalleriesData($request->get('galleries'))
    ...
    {
        try {
            Brand::findOrFail($request->get('id'))->update(['status' => $request->get('status')]);
        } catch (\Exception $ex) {
            return ServiceResponse::jsonNotification($ex->getMessage(), $ex->getCode(), []);
        }
